__credits__ = ["Darius M"]
'''
Only for some evaluation metrics.
'''
...
class LunarLanderCustom(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def step(self, action):
        ...
        # if it is in a range on Ox axis
        if -0.2 < state[0] < 0.2:
            reward += 0.05
            cond1 = True
        else:
        ...
        # if it is in a range on Oy axis
        if -0.1 < state[1] < 0.5:
            reward += 0.1
            if state[1] < 0.1:
                reward += 0.05
        ...
        if angle_targ > 0.3 or angle_targ < -0.3:  # if the ship angle is more than 17 degree
            reward -= 2

        # if the horizontal speed is too high
        if abs(state[2]) > 0.3:
            reward -= 1

        # if it falls too hard or starts to rise a little
        if state[3] < -0.5 or state[3] > 0.02:
            reward -= 0.75

        terminated = False
        if self.game_over or abs(state[0]) >= 1.0:
            terminated = True
            reward -= 1000  # modified from -300 when training
            # print("Crash")
        if state[6] == state[7] == 1:
            terminated = True
            if cond1 and cond2:
                reward += 200  # modified from +100
                if -0.15 <= state[0] <= 0.15:
                    reward += 50
                else:
                    reward -= 50
