...
actions_freq = {1: 0, 2: 0, 3: 0}
last_10_actions = [0 for _ in range(10)]
...
class LunarLanderCustom(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
            ...
            wind_power: float = random.uniform(0, 15),
            turbulence_power: float = random.uniform(0, 1.5),
    ):
        EzPickle.__init__(
            self,
            render_mode,
    ...
    def reset(
        ...
        # modificari
        self.wind_power= random.uniform(0, 15)
        self.turbulence_power= random.uniform(0, 1.5)

        # print("Wind power: ", self.wind_power, "Turbulence power: ", self.turbulence_power)
        # modificari
    ...
    def step(self, action):
        ...
        if action != 0:
            ...
            if last_10_actions.count(action) > 4:
                for i in range(len(last_10_actions)):
                    if last_10_actions[i] == action:
                        last_10_actions[i] = 0
                        # print("Action changed 1")
                action = 0
            if action != 0:
                if actions_freq[action] >= 15:
                    actions_freq[action] = 0
                    action = 0;
                    # print("Action changed 2 ---")
